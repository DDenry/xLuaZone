...
function StudioData.FindStudioObjectData()
    ...
    print("Do: Find Studio ObjectData~")

    SenseStudioNameSpace = _Global:GetData("SenseStudioNameSpace")
    ...
    if SenseStudioNameSpace == "SceneStudio.StudioDataManager" then
        ...
        if objects.Length > 0 then
            ...
            for i = 0, objects.Length - 1 do
                ...
                animationGameObjectId[i] = tostring(objects[i].gameObject:GetComponent(typeof(objectDataType)).ID)
    ...
    elseif SenseStudioNameSpace == "SenseStudio.IV.StudioDataManager" then
        ...
        if objects.Length > 0 then
            ...
            for i = 0, objects.Length - 1 do
                ...
                animationGameObjectId[i] = tostring(objects[i].gameObject:GetComponent(typeof(objectDataType)).Guid)
...
function StudioData.HandleAnimation()
    ...
    --判断是否存在爆炸动画
    for i = 0, timeLines.Count - 1 do
        if timeLines[i].Name == "爆炸" then
            if timeLines[i].Groups.Count > 0 then
                --标记爆炸动画步骤
                BoomStep = i
    ...
    --判断剩余的动画
    if timeLines.Count > (((MENU.functionsList['BOOM'] and 1) or 0) + ((MENU.functionsList['SELF_ANIMATION'] and 1) or 0)) then
        --获取拆装步数
        step = timeLines.Count - (((MENU.functionsList['BOOM'] and 1) or 0) + ((MENU.functionsList['SELF_ANIMATION'] and 1) or 0))
        --
        if step <= 1 then
            --判断是否存在有意义的动画数据
            if timeLines[0].Groups.Count > 0 then
                --标记 havePart = true
...
function PROCESS.RestoredModel()
    ...
    elseif _Global:GetData("SenseStudioNameSpace") == "SenseStudio.IV.StudioDataManager" then
        --应用场景第一步状态
        local sceneData = CS.SenseStudio.IV.StudioDataManager.Instance.CurrentScene.SceneData

        sceneData.TimelineData:ApplyTimeline(sceneData.TimelineData.SceneTimelineData[0]);
    end
