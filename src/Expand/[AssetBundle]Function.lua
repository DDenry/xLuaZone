...
function StudioData.FindStudioObjectData()
    ...
    local objects = GameObject.Find("Root/Models/Contents/Prefab-ModelLoader/MakerAll"):GetComponentsInChildren(typeof(CS.ObjectData), true)
    if objects.Length > 0 then
        --遍历模型物体和ID
        for i = 0, objects.Length - 1 do
            animationGameObject[i] = objects[i].gameObject
            animationGameObjectId[i] = objects[i].gameObject:GetComponent(typeof(CS.ObjectData)).ID
...
function StudioData.LoadPointData()
    ...
    --TODO:解析point.data
    local www = CS.UnityEngine.WWW(wwwAssetPath .. sceneName .. "/point.data")

    yield_return(www)

    if www.error == nil then
        local pointsData = JSON.Parse(www.text)

        if pointsData[1].Count > 0 then
            --遍历标注点信息
            for i = 0, pointsData[1].Count - 1 do
                local _id
                local name
                _, _, _, _id = string.find(tostring(pointsData[1][i][0]), "([\"'])(.-)%1")
                _, _, _, name = string.find(tostring(pointsData[1][i][1]), "([\"'])(.-)%1")

                table_point[i] = pointsData[1][i]
            end
            ...
            for i = 0, #table_point do
                ...
                local _id
                _, _, _, _id = string.find(tostring(table_point[i][0]), "([\"'])(.-)%1")
                local transform = _Global:GetData(_id).transform

                Prefab_Point_Tmp.transform:SetParent(Prefab_Point.transform.parent)
                --
                Prefab_Point_Tmp:GetComponent(typeof(CS.EzComponents.UnityUI.TransformPointTracker)).TargetTransform = transform
                Prefab_Point_Tmp:GetComponent(typeof(CS.EzComponents.DepthCulling)).TargetTransform = transform
                Prefab_Point_Tmp:GetComponent(typeof(CS.EzComponents.TransformSetter)).Value = transform
                Prefab_Point_Tmp:GetComponent(typeof(CS.EzComponents.TransformSetter)):Set()

                local name
                _, _, _, name = string.find(tostring(table_point[i][1]), "([\"'])(.-)%1")
                ...
                if (name ~= nil) and (name:trim() ~= "") then
                    ...
                    if (string.sub(name, 1, 1) ~= "#") then
                        --
                        Prefab_Point_Tmp:GetComponent(typeof(CS.EzComponents.StringSetter)).Value = "   " .. name .. "   "

                    else
                        Prefab_Point_Tmp:GetComponent(typeof(CS.EzComponents.StringSetter)).Value = string.gsub(name, "#", "")
                    end
                else
                ...
                --设置标注点默认H5页
                local point_url = wwwAssetPath .. sceneName .. "/" .. _Global:GetData(_id):GetComponent(typeof(CS.ObjectData)).ID .. ".html"
...
function PROCESS.InitDataTable()
    ...
    defaultViewTransform = {
        ...
        --初始模型角度
        localRotation = Quaternion.Euler(Vector3(0, -90, 0)),
