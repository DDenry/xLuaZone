...
local WWWAssetBundleLoader = Root.transform:Find("Models/Contents/Prefab-ModelLoader"):GetComponent(typeof(CS.EzComponents.WWWAssetbundleLoader))
...
function PROCEDURE.StartScanModelScene()
    ...
    local COROUTINE_ShowLoadingProcess = coroutine.create(function()
        ...
        COMMON.RegisterListener(WWWAssetBundleLoader.OnLoaded, function()
            --注销回调监听
            COMMON.UnregisterListener(WWWAssetBundleLoader.OnLoaded)
        ...
        assert(coroutine.resume(coroutine.create(function()
            ...
            yield_return(1)
            --
            WWWAssetBundleLoader:Load(wwwAssetPath .. sceneName .. "/assetbundle")
        end)))
    end)
...
function CALLBACK.ModelLoaded(...)
    ...
    if trackerType == "Once" or loadedType == 1 then
        ...
        for i = 0, MakerAll:GetComponentsInChildren(typeof(CS.ObjectData)).Length - 1 do
            MakerAll:GetComponentsInChildren(typeof(CS.ObjectData))[i].gameObject.layer = 8
        end
